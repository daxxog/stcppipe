...
#include "acpdump2.h"
    ...
    #define INTERLOCK_GET(X)    X // InterlockedExchangeAdd(&X, 0)
...
void handle_connections(int sock, int sd_one, int *sd_array);
quick_thread(double_client, thread_sock *t_sock);
quick_thread(client, int sock);
quick_thread(multi_connect, int sock);
...
int main(int argc, char *argv[]) {
    ...
    if(argc < 4) {
        ...
        fprintf(stderr, "\n"
            ...
            "         local_port with the byte 0x%02x, 2 for dest_port or 3 for both\n"
    ...
    for(;;) {
        ...
        if(!quick_threadx(do_multi ? multi_connect : client, (void *)sdla)) {
            close(sdla);
...
#define MULTI_SKIP_QUIT \
    ...
    if(dossl) {
        ...
        if(stristr(ssl_method_type, "23"))      ssl_method = (SSL_METHOD *)SSLv23_method();
        if(stristr(ssl_method_type, "2"))       ssl_method = (SSL_METHOD *)SSLv2_method();
        if(stristr(ssl_method_type, "3"))       ssl_method = (SSL_METHOD *)SSLv3_method();
        if(stristr(ssl_method_type, "dtls"))    ssl_method = (SSL_METHOD *)DTLSv1_method();
        if(stristr(ssl_method_type, "1"))       ssl_method = (SSL_METHOD *)TLSv1_method();
        if(!ssl_method_type)                    ssl_method = (SSL_METHOD *)SSLv23_method();
    }
...
quick_thread(double_client, thread_sock *t_sock) {
    handle_connections(t_sock->local_sock, t_sock->dest_sock, NULL);
    return(0);
}
...
quick_thread(client, int sock) {
    ...
    handle_connections(sock, sd, NULL);
    return(0);
}



quick_thread(multi_connect, int sock) { // EXACTLY as above!
    struct sockaddr_in  peer_tmp;
    int     *sd,
            i,
            socks;
